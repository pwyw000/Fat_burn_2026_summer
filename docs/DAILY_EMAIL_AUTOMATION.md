...
3. **维护 CSV**  
   ...
   - 周日：写 `logs/weekly_review.csv`（对照 **−0.5% 体重/周** 目标）。  
   - 提交并推送 CSV + 计划（若用户要求 commit）。

4. **检索网络减脂建议**（可信来源），按个人数据裁剪；禁止极端节食。

5. **生成当日计划** → `logs/plans/YYYY-MM-DD.md`  
   硬目标：**每周约 −0.5% 体重（脂肪为主）**；体重 **≥ 160 lb**；**Slow Carb + 周六 Cheat**；**2 跑 4 力**（课表见下）。

6. **发信**（用户触发时）到 `pwyw000@gmail.com`（`GMAIL_USER` / `GMAIL_APP_PASSWORD`）。

## 固定周训练（邮件须整表贴出 + 写「今日练什么」）

来源：`docs/WEEK1_PLAN.md` §2 · `docs/weekly_workout_split.jpg`。

| 日 | 训练 | 今日邮件要写的部位 |
|---|---|---|
| 周日 | **跑步** | 轻松跑 · 目标均 HR 155–165 |
| 周一 | **力量 20–30 min 超级组** | **Shoulders + arms** · 午饭前优先 |
| 周二 | **力量 20–30 min 超级组** | **Chest + abs** · 午饭前优先 |
| 周三 | **跑步** | 周中跑 |
| 周四 | **力量 20–30 min 超级组** | **Back + rear delts** · 午饭前优先 |
| 周五 | **力量 20–30 min 超级组** | **Lower body** · 午饭前优先 |
| 周六 | **休息 + Cheat** | **Cheat 餐前深蹲 3×10** |

力量时机：**午饭前最好，晚饭前次之。** 黄/红：减组不缺席、不砍热量。

## 能量与饮食（邮件必写）

- 引用 **Whoop 日常消耗 2,100 · 力量 +100 · 周赤字÷6（W×0.5%×3500）**。  
- **Slow Carb：套用 `WEEK1_PLAN.md` §3.1（力量日 ~1,700）或 §3.2（跑日 ~2,200）**；清单须含 **0% 希腊酸奶 170 g**；豆腐按 §3.3 / 主蛋白 E。禁止空泛建议。  
- 昨日 Nutrition 有缺口 → 在清单开头写一行 **「昨日差 X g 蛋白 / Y kcal → 今日加码：…」**，并对照 §3.3（**先酸奶/豆腐**，不要先砍热量）。  
- **蛋白优先于加深缺口：** 力量日目标仍是 **1,600–1,800**（不是 1,200）；未到 160 g 蛋白时加酸奶/豆腐，不扩大赤字。  
- **周六 Cheat Day**：邮件写放开说明 + **餐前深蹲** + **周日回归跑日 §3.2 清单**。  
...
| 日类型 | 套用模板 | 目标 kcal |
|---|---|---:|
| 纯休 Slow Carb 日 | §3.1 豆略减 | 1,500–1,700 |
| 力量日（Mon/Tue/Thu/Fri） | **§3.1** | 1,600–1,800（Whoop 日常 2100 + 力 100） |
| 跑步日（Sun/Wed） | **§3.2** | 2,150–2,350 |
...
## 4. 今日 Slow Carb 清单（[§3.1 力量日 ~1,700 / §3.2 跑日 ~2,200]）
...
|---|---|
| 周日 | 跑 |
| 周一 | 肩 + 臂 · 20–30 min 超级组 |
| 周二 | 胸 + 腹 · 20–30 min 超级组 |
| 周三 | 跑 |
| 周四 | 背 + 后束 · 20–30 min 超级组 |
| 周五 | 下肢 · 20–30 min 超级组 |
...
|---|---|
| 有氧 | [跑 / 不跑] |
| 阻力 | [部位 · 20–30 min 超级组 · 午饭前 / 晚饭前] |
| 示例动作 | [当日固定超级组，见 WEEK1_PLAN §2，不要加一对] |
| 恢复 | [Whoop 颜色 + 睡眠] |
...
3. **周六**：不写 kcal 上限；写「Cheat 放开 + 餐前深蹲 3×10 + 周日早餐起恢复跑日 §3.2」。  
4. **周日（Cheat 次日）**：用 §3.2（跑日），备注「不因晨重 spike 砍热量」。  
...
| **阻力** | training：本周是否朝 **4×/周**（肩臂/胸腹/背后束/下肢）推进？ |
