...
**工作区（iCloud Drive，手机可搜）：**  
`~/Library/Mobile Documents/com~apple~CloudDocs/Fat_burn_2026_summer`  
iPhone「文件」→ **iCloud 云盘** → **Fat_burn_2026_summer** → `logs/...`  
首次迁移步骤见 [docs/ICLOUD_WORKSPACE.md](docs/ICLOUD_WORKSPACE.md)。
...
4. 本机 LaunchAgent：**每天 07:55** 自动 push **截图 + `logs/plans/` + CSV**（不含文档/脚本）。脚本：`~/Library/Application Support/fatburn/auto-commit-push-logs.sh`（仓库本身在 iCloud）
...
| `docs/ICLOUD_WORKSPACE.md` | 把仓库挪进 iCloud、手机保存截图 |
| `docs/` | 档案、基线计划、调整规则 |
